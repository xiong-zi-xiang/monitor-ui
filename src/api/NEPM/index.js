...
export function changeGridState(logId, state) {
  return axiosInstance.put(
    baseURL + '/api/v1/grid-managers/state' + '?logId=' + logId + '&state=' + state
  )
}
...
export function getFeedbackInfo(
  ...
  if (assignDateAscending !== '' && assignDateAscending !== null)
    params.assignDateAscending = assignDateAscending
  // problems?
...
// 分条件查询统计数据
export function selectStatistic(
  page,
  size,
  id,
  afId,
  ...
  if (address !== '' && address !== null && address !== undefined) params.address = address
  //
...
export function enableAndDisableRole(roleId, enabled) {
  return axiosInstance.put(
    baseURL + `/api/v1/roles/${roleId}/enabled`,
    {
